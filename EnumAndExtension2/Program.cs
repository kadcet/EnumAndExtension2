...
class Program
{
    ...
    private static void KisiKayitlari()
    {
            ...
            var kisilerTablosu = new List<KisiKarti>
        {
            ...
            Console.WriteLine($"Ad: {gelenKayit.Ad} Soyad: {gelenKayit.Soyad} Sermaye Tutarı:{gelenKayit.Sermaye}  Yazı ile:{gelenKayit.Sermaye.ParayiYaziyaCevir()}");
            ...
            foreach (var item in sermayeyeGoreGelen)
            {
                ...
                var yazi = Convert.ToDecimal(item.Sermaye).ParayiYaziyaCevir();
